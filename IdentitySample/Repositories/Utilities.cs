...
namespace IdentitySample.Repositories
{
    public class Utilities : IUtilities
    {
        ...
        public Utilities(ApplicationDbContext dbContext)
        {
        ...
        public IList<string> GetAllAreasNames()
        {
            ...
            var contradistinction = asm.GetTypes()
                ...
                .Select(x => new
                {
                    Area = x.DeclaringType?.CustomAttributes.Where(c =>
                        c.AttributeType == typeof(AreaAttribute))
                });
            ...
            foreach (var item in contradistinction)
            {
                list.Add(item.Area.Select(v =>
                    v.ConstructorArguments[0].Value.ToString()).FirstOrDefault());
            }
        ...
        public string DataBaseRoleValidationGuid()
        {
            ...
            //تا زمانیکه entity در دیتابیس قرار نگرفته تکرار میشود
